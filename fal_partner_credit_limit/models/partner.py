    ...
    def _compute_get_oldest_invoice(self):
        ...
        for item in self:
            inv = self.env['account.move.line'].search([
                ('partner_id', '=', item.id), ('account_id.user_type_id', '=', self.env.ref('account.data_account_type_receivable').id), ('move_id.invoice_payment_state', '!=', 'paid')],
                order="date_maturity asc", limit=1)
            item.fal_oldest_invoice_no = inv.id
            duration = 0
            if inv.date_maturity and inv.move_id.invoice_payment_state != 'paid':
